...
// Mirrors lnd's own replace. lnrpc's rpc_utils.go uses protojson's
// UseHexForBytes option, which only exists in Lightning Labs' fork; a
// dependency's replace directive does not apply transitively, so the main
// module has to restate it or lnrpc will not compile.
