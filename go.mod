module github.com/AusDavo/winthistle
...
// Mirrors lnd's own replace, and the version has to be kept level with it by
// hand: lnrpc's rpc_utils.go uses protojson's UseHexForBytes option, which only
// exists in Lightning Labs' fork, and a dependency's replace directive does not
// apply transitively, so the main module has to restate it or lnrpc will not
// compile. Because it is a restatement rather than an inheritance, `go get` on
// lnd does not move it — this line said v1.30.0-hex-display against lnd
// v0.21.2-beta, which replaces at v1.33.0-hex-display. Check
// lnd's own go.mod on every version bump.
